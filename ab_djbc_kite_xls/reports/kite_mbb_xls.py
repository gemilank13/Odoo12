...
class KiteMbbXlsx(models.AbstractModel):
    _name = 'report.ab_djbc_kite_xls.kite_mbb_xlsx'
    _inherit = 'report.report_xlsx.abstract'

    def generate_xlsx_report(self, workbook, data, lines):
        # raise UserError("yes working...")
        sheet = workbook.add_worksheet('DJBC Laporan Mutasi Bahan Baku')
        ...
        sheet.merge_range('A1:N1', 'DJBC Laporan Mutasi Bahan Baku', format1)
        ...
        no = 1
        row = 4
        lines = self.env['djbc.kite_mbb'].search([])
        for obj in lines:
            sheet.write(row, 0, no, format2)
            sheet.write(row, 1, obj.kode_barang, format2)
            sheet.write(row, 2, obj.nama_barang, format4)
