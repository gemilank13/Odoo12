...
class DJBCMhp(models.Model):
    _name='djbc.kite_mhp'
    _description='DJBC Laporan Mutasi Hasil Produksi'
    ...
    def init(self):
        self.env.cr.execute("""
        DROP FUNCTION IF EXISTS djbc_kite_mhp(DATE, DATE);
        CREATE OR REPLACE FUNCTION djbc_kite_mhp(date_start DATE, date_end DATE)
...
BEGIN
	...
	v_date_end = date_end;
	delete from djbc_kite_mhp;


		insert into djbc_kite_mhp (kode_barang, nama_barang, saldo_awal, pemasukan, pengeluaran, penyesuaian, stock_opname, saldo_akhir, selisih, keterangan, location, warehouse, tgl_mulai, tgl_akhir)
