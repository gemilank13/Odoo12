...
class DJBCMbb(models.Model):
    _name='djbc.kite_mbb'
    _description='DJBC Laporan Mutasi Bahan Baku'
    ...
    def init(self):
        self.env.cr.execute("""
        DROP FUNCTION IF EXISTS djbc_kite_mbb(DATE, DATE);
        CREATE OR REPLACE FUNCTION djbc_kite_mbb(date_start DATE, date_end DATE)
...
BEGIN
	...
	v_date_end = date_end;
	delete from djbc_kite_mbb;


		insert into djbc_kite_mbb (kode_barang, nama_barang, saldo_awal, pemasukan, pengeluaran, penyesuaian, stock_opname, saldo_akhir, selisih, keterangan, location, warehouse, tgl_mulai, tgl_akhir)
